...
- forEach는 배열의 각 아이템을 하나씩 가져와서 작업을 처리합니다.
- 각 아이템의 순서인 index도 인자로 받아올 수 있습니다.
- 1번에 담아두었던 이미지를 0~5까지의 랜덤한 숫자를 출력하는 함수를 만들어 src를 가져와서 forEach 반복문으로 돌려주는 각각의 이미지 태그에 소스값으로 넣어둡니다.
